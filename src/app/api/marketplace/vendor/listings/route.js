...
import { createListing, listVendorListings } from "@/lib/marketplace/listings.js";
import { getAuthenticatedVendor } from "@/lib/marketplace/vendor-session.js";
import { withRequestLogging } from "@/lib/server-logger";

export const runtime = "nodejs";

// The signed-in vendor's own listings (for the phone app's seller view).
export async function GET(request) {
    return withRequestLogging(request, "GET /api/marketplace/vendor/listings", async ({ internalError }) => {
        try {
            ...
            return internalError(error, { event: "marketplace.vendor.listings.list.failure" });
        }
    });
}

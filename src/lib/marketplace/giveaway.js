...
export async function giveawayChest({ tier = "wooden", count = 1 } = {}) {
    ...
    for (const id of ids) await addChests(id, { [tier]: n });
    await pushAll(ids, {
...
export async function giveawayItem({ itemId } = {}) {
    ...
    const ids = await activeHeroIds();
    for (const id of ids) await grantItem(id, item.id, "giveaway").catch(() => {});
...
export async function giveawayGold({ amount = 100 } = {}) {
    ...
    const ids = await activeHeroIds();
    for (const id of ids) await db.query(`UPDATE mkt_buyer SET gold = gold + $2 WHERE id = $1`, [id, gold]).catch(() => {});
    await pushAll(ids, {
        title: "🪙 Free gold!",

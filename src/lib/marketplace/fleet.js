...
//
// `art` is the mkt_town_art key for the hull sprite (generated by scripts/gen-fleet-ships.mjs). Missing art
// falls back to the player's own boat sprite mirrored, so the fleet is playable before it is painted.
...
export function fleetView(depth = 0) {
    ...
    return FLEET.filter((f) => f.rank <= next).map((f) => ({
        rank: f.rank, name: f.name, cls: f.cls, art: f.art, boss: Boolean(f.boss), flavor: f.flavor,

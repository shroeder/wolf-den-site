...
// ── THERE IS NO CROP KEEP-OUT, AND THERE CANNOT BE ONE ───────────────────────────────────────────────────────
// `nearPlots` above is dead, and it is dead for a better reason than "nobody called it": a decoration and a crop
// bed CANNOT SHARE A SCENE. The farm has three separate views — outside, inside, garden — decorations are
// filtered to the view they were placed in, and placeDecoration normalises `view` to outside/inside and never
// garden (see `vw` below). ScenePlots only renders under `view === "garden"`. The two never coexist.
//
// I briefly shipped a guard here that refused to place an oversized piece low in the field "because the crops
// would cover it", and it fired on an empty pasture, citing crops that were not in the scene and could not be.
// If you are tempted to add one back: check which VIEW you are reasoning about first.
//

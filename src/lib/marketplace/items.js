...
export const ITEM_SOURCE_LABEL = {
    ...
    // Every admin item that survives carries a chargeReward, so this says the one true thing: you own it
    // already and you hand it IN. It used to read "Handed over the counter in the shop" across 44 items,
...
// for a source — which is how 41 items came to advertise a trip to the counter for nothing.
export function itemSourceLabel(item) {
    return item?.source ? ITEM_SOURCE_LABEL[item.source] || null : null;
}
...
export const REWARDS = {
    ...
    store_credit_20: "$20 store credit",
...
export const ITEMS = [
    ...
    // ===== THE THREE REAL REDEMPTIONS (source: "admin") — an item you already own, handed IN at the
    // counter to burn a charge for store credit. Forty-one siblings were deleted on 2026-09-03: they were
    // trophies whose names promised a real-world thing (a binder, a playmat, a skipped queue) that was
    // never wired to anything, and members read the Compendium line as an invitation to come and ask.
    // ⚠️ Do not add another one without a chargeReward. A perk with nothing behind it is a promise the
    // counter has to keep. =====


    { id: "starter_pack_charm", name: "Starter Pack Charm", slot: "amulet", rarity: "rare", icon: "GiFeatherNecklace", flavor: "Everyone starts somewhere.", stats: { fortune: 12 }, reqLevel: 10, source: "admin", charged: true, charges: 2, cooldownDays: 30, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 523 },

    { id: "linecutter_token", name: "Line-Cutter Token", slot: "ring", rarity: "common", icon: "GiSwirlRing", flavor: "Straight to the front.", stats: { might: 6, crit_chance: 4 }, reqLevel: 5, source: "admin", charged: true, charges: 3, cooldownDays: 14, chargeReward: "store_credit_5", chargeRewardLabel: REWARDS.store_credit_5, sort: 532 },
    ...
    // -- Ascendant --
    // THE REAL-WORLD REWARD DOES NOT LIVE ON THIS TIER ANY MORE — see the celestial and primordial blocks
    // below. Ascendant is the FOURTH-rarest tier and, since the chest work, the reachable one: a top player
    // sees an ascendant item about every eighteen months and somebody in the Den sees one every four weeks.
    // That is the right cadence for a chase item and the wrong one for something that redeems at the till.
    { id: "ascendant_crown", name: "Ascendant Crown", slot: "helmet", rarity: "ascendant", icon: "GiCrenelCrown", flavor: "Worn by those who rose past legend.", stats: { might: 20, crit_chance: 15, crit_power: 20 }, reqLevel: 80, source: "elite", sort: 900 },
    { id: "ascendant_blade", name: "Ascendant Blade", slot: "main_hand", rarity: "ascendant", icon: "GiEnergySword", flavor: "It hums with a light that shouldn't exist.", stats: { might: 35, crit_power: 20 }, reqLevel: 82, source: "elite", sort: 901 },
    ...
    // ── THE TOP TWO TIERS ARE THE ONES THAT PAY AT THE TILL ──────────────────────────────────────────
    // Luke: "we probably only want anywhere from 10 to $20 store credit and it should be distributed
    // between the top two tiers of items."
    //
    // It used to hang off two ASCENDANT pieces at $100 and $30, which was the wrong tier twice over: too
    // reachable to be a landmark, and too expensive to be safe once it became reachable at all. Ascendant is
    // fourth-rarest; celestial and primordial are the top of the ladder and the only two rarities a member
    // may never see. So every one of them carries something real, and the amount is small on purpose.
    //
    // ONE CHARGE, not two. These are meant to be a thing that happened once, not an annuity.
    //
    // The exposure is nothing: measured against the Den's actual pace, a celestial item reaches somebody
    // about every seventeen months and a primordial about every six years — call it eleven dollars a year
    // across the whole membership, against the two hundred and sixty that used to hang off two ascendants.
    { id: "celestial_celestial_blade", name: "Celestial Blade", slot: "main_hand", rarity: "celestial", icon: "GiFlangedMace", flavor: "Cut from something that was falling.", stats: { might: 55, crit_power: 45 }, reqLevel: 105, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1063 },
    { id: "celestial_celestial_bulwark", name: "Celestial Bulwark", slot: "off_hand", rarity: "celestial", icon: "GiRoundShield", flavor: "It keeps the cold of very high places.", stats: { ferocity: 60, fortune: 40 }, reqLevel: 105, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1064 },
    { id: "celestial_celestial_diadem", name: "Celestial Diadem", slot: "helmet", rarity: "celestial", icon: "GiOverlordHelm", flavor: "There is a sky inside it.", stats: { crit_chance: 50, might: 50 }, reqLevel: 105, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1065 },
    { id: "celestial_celestial_shroud", name: "Celestial Shroud", slot: "chest", rarity: "celestial", icon: "GiScaleMail", flavor: "It answers to a longer year than ours.", stats: { ferocity: 65, might: 35 }, reqLevel: 106, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1066 },
    { id: "celestial_celestial_binding", name: "Celestial Binding", slot: "belt", rarity: "celestial", icon: "GiPowderBag", flavor: "Cut from something that was falling.", stats: { might: 60, ferocity: 40 }, reqLevel: 106, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1067 },
    { id: "celestial_celestial_walkers", name: "Celestial Walkers", slot: "boots", rarity: "celestial", icon: "GiRunningShoe", flavor: "It keeps the cold of very high places.", stats: { crit_chance: 60, ferocity: 40 }, reqLevel: 106, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1068 },
    { id: "celestial_celestial_pinions", name: "Celestial Pinions", slot: "back", rarity: "celestial", icon: "GiWingedScepter", flavor: "There is a sky inside it.", stats: { fortune: 55, crit_chance: 45 }, reqLevel: 107, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1069 },
    { id: "celestial_celestial_medallion", name: "Celestial Medallion", slot: "amulet", rarity: "celestial", icon: "GiGemPendant", flavor: "It answers to a longer year than ours.", stats: { crit_power: 45, fortune: 30, crit_chance: 25 }, reqLevel: 107, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1070 },
    { id: "celestial_celestial_bond", name: "Celestial Bond", slot: "ring", rarity: "celestial", icon: "GiRing", flavor: "Cut from something that was falling.", stats: { might: 35, crit_chance: 35, fortune: 30 }, reqLevel: 107, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1071 },
    { id: "celestial_starbound_sabre", name: "Starbound Sabre", slot: "main_hand", rarity: "celestial", icon: "GiRuneSword", flavor: "It keeps the cold of very high places.", stats: { might: 55, crit_power: 45 }, reqLevel: 108, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1072 },
    { id: "celestial_starbound_wall", name: "Starbound Wall", slot: "off_hand", rarity: "celestial", icon: "GiSpellBook", flavor: "There is a sky inside it.", stats: { ferocity: 60, fortune: 40 }, reqLevel: 108, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1073 },
    { id: "celestial_starbound_veil", name: "Starbound Veil", slot: "helmet", rarity: "celestial", icon: "GiQueenCrown", flavor: "It answers to a longer year than ours.", stats: { crit_chance: 50, might: 50 }, reqLevel: 108, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1074 },
    { id: "celestial_starbound_plate", name: "Starbound Plate", slot: "chest", rarity: "celestial", icon: "GiSpikedArmor", flavor: "Cut from something that was falling.", stats: { ferocity: 65, might: 35 }, reqLevel: 109, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1075 },
    { id: "celestial_starbound_sash", name: "Starbound Sash", slot: "belt", rarity: "celestial", icon: "GiBelt", flavor: "It keeps the cold of very high places.", stats: { might: 60, ferocity: 40 }, reqLevel: 109, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1076 },
    { id: "celestial_starbound_greaves", name: "Starbound Greaves", slot: "boots", rarity: "celestial", icon: "GiBootStomp", flavor: "There is a sky inside it.", stats: { crit_chance: 60, ferocity: 40 }, reqLevel: 109, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1077 },
    { id: "celestial_starbound_cape", name: "Starbound Cape", slot: "back", rarity: "celestial", icon: "GiAngelWings", flavor: "It answers to a longer year than ours.", stats: { fortune: 55, crit_chance: 45 }, reqLevel: 110, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1078 },
    { id: "celestial_starbound_charm", name: "Starbound Charm", slot: "amulet", rarity: "celestial", icon: "GiHeartNecklace", flavor: "Cut from something that was falling.", stats: { crit_power: 45, fortune: 30, crit_chance: 25 }, reqLevel: 110, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1079 },
    { id: "celestial_starbound_coil", name: "Starbound Coil", slot: "ring", rarity: "celestial", icon: "GiSkullRing", flavor: "It keeps the cold of very high places.", stats: { might: 35, crit_chance: 35, fortune: 30 }, reqLevel: 110, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1080 },
    { id: "celestial_astral_cleaver", name: "Astral Cleaver", slot: "main_hand", rarity: "celestial", icon: "GiSickle", flavor: "There is a sky inside it.", stats: { might: 55, crit_power: 45 }, reqLevel: 111, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1081 },
    { id: "celestial_astral_orb", name: "Astral Orb", slot: "off_hand", rarity: "celestial", icon: "GiShieldBash", flavor: "It answers to a longer year than ours.", stats: { ferocity: 60, fortune: 40 }, reqLevel: 111, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1082 },
    { id: "celestial_astral_hood", name: "Astral Hood", slot: "helmet", rarity: "celestial", icon: "GiBarbute", flavor: "Cut from something that was falling.", stats: { crit_chance: 50, might: 50 }, reqLevel: 111, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1083 },
    { id: "celestial_astral_scale", name: "Astral Scale", slot: "chest", rarity: "celestial", icon: "GiChestArmor", flavor: "It keeps the cold of very high places.", stats: { ferocity: 65, might: 35 }, reqLevel: 112, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1084 },
    { id: "celestial_astral_waistguard", name: "Astral Waistguard", slot: "belt", rarity: "celestial", icon: "GiBeltArmor", flavor: "There is a sky inside it.", stats: { might: 60, ferocity: 40 }, reqLevel: 112, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1085 },
    { id: "celestial_astral_tracks", name: "Astral Tracks", slot: "boots", rarity: "celestial", icon: "GiLegArmor", flavor: "It answers to a longer year than ours.", stats: { crit_chance: 60, ferocity: 40 }, reqLevel: 112, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1086 },
    { id: "celestial_astral_cloak", name: "Astral Cloak", slot: "back", rarity: "celestial", icon: "GiBatwingEmblem", flavor: "Cut from something that was falling.", stats: { fortune: 55, crit_chance: 45 }, reqLevel: 113, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1087 },
    { id: "celestial_astral_amulet", name: "Astral Amulet", slot: "amulet", rarity: "celestial", icon: "GiIntricateNecklace", flavor: "It keeps the cold of very high places.", stats: { crit_power: 45, fortune: 30, crit_chance: 25 }, reqLevel: 113, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1088 },
    { id: "celestial_astral_signet", name: "Astral Signet", slot: "ring", rarity: "celestial", icon: "GiSkullSignet", flavor: "There is a sky inside it.", stats: { might: 35, crit_chance: 35, fortune: 30 }, reqLevel: 113, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1089 },
    { id: "celestial_empyrean_scythe", name: "Empyrean Scythe", slot: "main_hand", rarity: "celestial", icon: "GiWarhammer", flavor: "It answers to a longer year than ours.", stats: { might: 55, crit_power: 45 }, reqLevel: 114, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1090 },
    { id: "celestial_empyrean_rampart", name: "Empyrean Rampart", slot: "off_hand", rarity: "celestial", icon: "GiCrystalCluster", flavor: "Cut from something that was falling.", stats: { ferocity: 60, fortune: 40 }, reqLevel: 114, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1091 },
    { id: "celestial_empyrean_coronet", name: "Empyrean Coronet", slot: "helmet", rarity: "celestial", icon: "GiVisoredHelm", flavor: "It keeps the cold of very high places.", stats: { crit_chance: 50, might: 50 }, reqLevel: 114, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_10", chargeRewardLabel: REWARDS.store_credit_10, sort: 1092 },
    { id: "primordial_primordial_blade", name: "Primordial Blade", slot: "main_hand", rarity: "primordial", icon: "GiWingedSword", flavor: "From before there was a word for it.", stats: { might: 72, crit_power: 59 }, reqLevel: 120, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1093 },
    { id: "primordial_primordial_bulwark", name: "Primordial Bulwark", slot: "off_hand", rarity: "primordial", icon: "GiSurroundedShield", flavor: "The first of its kind, and still the last.", stats: { ferocity: 78, fortune: 52 }, reqLevel: 120, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1094 },
    { id: "primordial_primordial_diadem", name: "Primordial Diadem", slot: "helmet", rarity: "primordial", icon: "GiCenturionHelmet", flavor: "It remembers the shape of the world underneath.", stats: { crit_chance: 65, might: 65 }, reqLevel: 120, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1095 },
    { id: "primordial_primordial_shroud", name: "Primordial Shroud", slot: "chest", rarity: "primordial", icon: "GiAbdominalArmor", flavor: "Older than the stone it was found in.", stats: { ferocity: 85, might: 46 }, reqLevel: 121, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1096 },
    { id: "primordial_primordial_binding", name: "Primordial Binding", slot: "belt", rarity: "primordial", icon: "GiBlackBelt", flavor: "From before there was a word for it.", stats: { might: 78, ferocity: 52 }, reqLevel: 121, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1097 },
    { id: "primordial_primordial_walkers", name: "Primordial Walkers", slot: "boots", rarity: "primordial", icon: "GiBootKick", flavor: "The first of its kind, and still the last.", stats: { crit_chance: 78, ferocity: 52 }, reqLevel: 121, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1098 },
    { id: "primordial_primordial_pinions", name: "Primordial Pinions", slot: "back", rarity: "primordial", icon: "GiCurlyWing", flavor: "It remembers the shape of the world underneath.", stats: { fortune: 72, crit_chance: 59 }, reqLevel: 122, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1099 },
    { id: "primordial_primordial_medallion", name: "Primordial Medallion", slot: "amulet", rarity: "primordial", icon: "GiTribalPendant", flavor: "Older than the stone it was found in.", stats: { crit_power: 59, fortune: 39, crit_chance: 33 }, reqLevel: 122, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1100 },
    { id: "primordial_primordial_bond", name: "Primordial Bond", slot: "ring", rarity: "primordial", icon: "GiSwirlRing", flavor: "From before there was a word for it.", stats: { might: 46, crit_chance: 46, fortune: 39 }, reqLevel: 122, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1101 },
    { id: "primordial_firstborn_sabre", name: "Firstborn Sabre", slot: "main_hand", rarity: "primordial", icon: "GiWizardStaff", flavor: "The first of its kind, and still the last.", stats: { might: 72, crit_power: 59 }, reqLevel: 123, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1102 },
    { id: "primordial_firstborn_wall", name: "Firstborn Wall", slot: "off_hand", rarity: "primordial", icon: "GiVibratingShield", flavor: "It remembers the shape of the world underneath.", stats: { ferocity: 78, fortune: 52 }, reqLevel: 123, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1103 },
    { id: "primordial_firstborn_veil", name: "Firstborn Veil", slot: "helmet", rarity: "primordial", icon: "GiLaurelCrown", flavor: "Older than the stone it was found in.", stats: { crit_chance: 65, might: 65 }, reqLevel: 123, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1104 },
    { id: "primordial_firstborn_plate", name: "Firstborn Plate", slot: "chest", rarity: "primordial", icon: "GiKimono", flavor: "From before there was a word for it.", stats: { ferocity: 85, might: 46 }, reqLevel: 124, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1105 },
    { id: "primordial_firstborn_sash", name: "Firstborn Sash", slot: "belt", rarity: "primordial", icon: "GiPowderBag", flavor: "The first of its kind, and still the last.", stats: { might: 78, ferocity: 52 }, reqLevel: 124, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1106 },
    { id: "primordial_firstborn_greaves", name: "Firstborn Greaves", slot: "boots", rarity: "primordial", icon: "GiWingfoot", flavor: "It remembers the shape of the world underneath.", stats: { crit_chance: 78, ferocity: 52 }, reqLevel: 124, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1107 },
    { id: "primordial_firstborn_cape", name: "Firstborn Cape", slot: "back", rarity: "primordial", icon: "GiFeatheredWing", flavor: "Older than the stone it was found in.", stats: { fortune: 72, crit_chance: 59 }, reqLevel: 125, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1108 },
    { id: "primordial_firstborn_charm", name: "Firstborn Charm", slot: "amulet", rarity: "primordial", icon: "GiPearlNecklace", flavor: "From before there was a word for it.", stats: { crit_power: 59, fortune: 39, crit_chance: 33 }, reqLevel: 125, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1109 },
    { id: "primordial_firstborn_coil", name: "Firstborn Coil", slot: "ring", rarity: "primordial", icon: "GiRingedBeam", flavor: "The first of its kind, and still the last.", stats: { might: 46, crit_chance: 46, fortune: 39 }, reqLevel: 125, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1110 },
    { id: "primordial_elder_cleaver", name: "Elder Cleaver", slot: "main_hand", rarity: "primordial", icon: "GiPlainDagger", flavor: "It remembers the shape of the world underneath.", stats: { might: 72, crit_power: 59 }, reqLevel: 126, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1111 },
    { id: "primordial_elder_orb", name: "Elder Orb", slot: "off_hand", rarity: "primordial", icon: "GiEyeShield", flavor: "Older than the stone it was found in.", stats: { ferocity: 78, fortune: 52 }, reqLevel: 126, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1112 },
    { id: "primordial_elder_hood", name: "Elder Hood", slot: "helmet", rarity: "primordial", icon: "GiWizardFace", flavor: "From before there was a word for it.", stats: { crit_chance: 65, might: 65 }, reqLevel: 126, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1113 },
    { id: "primordial_elder_scale", name: "Elder Scale", slot: "chest", rarity: "primordial", icon: "GiLegArmor", flavor: "The first of its kind, and still the last.", stats: { ferocity: 85, might: 46 }, reqLevel: 127, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1114 },
    { id: "primordial_elder_waistguard", name: "Elder Waistguard", slot: "belt", rarity: "primordial", icon: "GiBelt", flavor: "It remembers the shape of the world underneath.", stats: { might: 78, ferocity: 52 }, reqLevel: 127, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1115 },
    { id: "primordial_elder_tracks", name: "Elder Tracks", slot: "boots", rarity: "primordial", icon: "GiBoots", flavor: "Older than the stone it was found in.", stats: { crit_chance: 78, ferocity: 52 }, reqLevel: 127, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1116 },
    { id: "primordial_elder_cloak", name: "Elder Cloak", slot: "back", rarity: "primordial", icon: "GiCondorEmblem", flavor: "From before there was a word for it.", stats: { fortune: 72, crit_chance: 59 }, reqLevel: 128, source: "elite", charged: true, charges: 1, cooldownDays: 365, chargeReward: "store_credit_20", chargeRewardLabel: REWARDS.store_credit_20, sort: 1117 },
...
// ── THE DRAW READS A FROZEN INDEX, NOT AN ARRAY POSITION ─────────────────────────────────────────────────────
// The draw weight is `hash(id:key) ^ (seed + i)`, and `i` used to be the position in the array below. That
// made the array impossible to edit: taking one entry out shifted every affix after it, silently redrawing
// stun and haste across the whole catalogue and changing gear members are already wearing. So `doublestrike`
// had to stay in the list long after the mechanic it was named for was gone.
//
// AFFIX_INDEX freezes each affix's `i` at the value it has always had. The order is now data rather than
// geometry: an affix can be removed, and every OTHER affix draws exactly as it did before.
//
// Luke: "we need to fully retire that." Doublestrike is now gone from the draw entirely rather than drawn and
// relabelled. Twenty-one pieces used to win a slot with it and take the points as Ferocity; those twenty-one
// now draw the next affix in their own order instead. Nothing else in the catalogue moves — verified by
// diffing every stat on all 425 items before and after.
const AFFIX_INDEX = {
    might: 0, crit_chance: 1, crit_power: 2, ferocity: 3, fortune: 4,
    vitality: 5, tenacity: 6, pierce: 7, lifesteal: 8, counter: 9,
    doublestrike: 10,   // ⚠️ KEEP THIS ENTRY. Nothing draws it any more, but the numbers below are the
                        // historical indices of stun and haste and they must not shift.
    stun: 11, haste: 12,
};
const AFFIX_DRAW_ORDER = ["might", "crit_chance", "crit_power", "ferocity", "fortune",
    "vitality", "tenacity", "pierce", "lifesteal", "counter", "stun", "haste"];
...
// Nothing is retired-but-drawn any more — doublestrike is out of AFFIX_DRAW_ORDER entirely. Kept as an
// empty map because grant() still reads it, and the next stat to be retired will want it.
const RETIRED_AFFIX = {};
...
const AFFIX_RARITY = {
    ...
    tenacity: 1.5, pierce: 3, lifesteal: 5, counter: 7,

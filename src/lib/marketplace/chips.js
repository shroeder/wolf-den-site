...
// ── SABLE SELLS THREE. THE MACHINES DROP FIVE MORE. ──────────────────────────────────────────────────────────
// Worth stating plainly because the two sets look alike and are not: the eight casino pets come from two
// different places and do two different things.
//
//   THESE THREE are bought, here, with chips. No casinoPerk — they are trophies, and what they carry is an
//   ordinary pet ability that works anywhere in the game.
//   THE OTHER FIVE (copper_paw, tallyman, gilded_magpie, croupiers_cat, night_auditor) cannot be bought at
//   all. They DROP from playing the machines at long odds, and each one carries a casinoPerk that makes the
//   floor itself kinder — free plays, better prizes, the odd loss pushed back. That is what "The Five" badge
//   is for, and it is why they are not on this shelf.
//
...
async function detailFor(item) {
    switch (item.kind) {
        ...
        case "pet": {
            ...
            // ── THE SAME CARD THE REST OF THE GAME DRAWS ─────────────────────────────────────────────
            // The first version of this printed `statLabel(c.activeStat)` against a row called "While it is
            // out", which produced the line "While it is out — Gold_find": a raw database key, title-cased by
            // accident, describing a stat rather than an ability. Luke: "show pets like we normally do, not
            // the weird text descriptions."
            //
            // Two things were wrong and only one of them was cosmetic. `activeStat` is a cosmetic farm stat;
            // the thing a pet actually DOES is its perk, authored in PET_PERKS and rendered everywhere else
            // in the game as an icon, a name and a sentence. This card was the only place inventing its own
            // vocabulary — and because it read activeStat it also went stale the moment a pet's real ability
            // was retuned, so the Ferret advertised gold_find for a while after it had stopped granting it.
            //
            // petPerk() is the one spelling of the answer. The Menagerie uses it, the level-up card uses it,
            // the enshrining panel uses it; now so does the shelf.
            const { petPerk } = await import("@/lib/marketplace/pet-perks.js");
            const perk = petPerk(c);
            return {
                art: sprite?.url || null,
                tone: c.color || null,
                blurb: c.hint ? `${c.hint}.` : null,
                lines: [
                    { label: "Rarity", value: RARITY_META?.[c.rarity]?.label || c.rarity, tone: RARITY_META?.[c.rarity]?.color || null },
                    // The ability by its NAME, the way it reads on the pet's own card.
                    perk?.name ? { label: perk.icon ? `${perk.icon} Ability` : "Ability", value: perk.name } : null,
                ].filter(Boolean),
                // And the sentence underneath, generated by the same function the Menagerie prints — so a
                // retune moves this card with it instead of leaving a second, wrong copy behind.
                foot: [perk?.desc, "Yours for good once bought. Equip it on the farm to have it follow you."]
                    .filter(Boolean).join(" "),
            };
        }
        // ── A PAGE ───────────────────────────────────────────────────────────────────

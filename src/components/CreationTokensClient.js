...
// Buy CREATION TOKENS — a token you spend in the farm's "Make your own" flow to design your own AI decoration
// (yours forever, non-tradeable). A purchase grants TOKENS + COINS, never store credit. Purple/pink theme,
...
export default function CreationTokensClient({
    ...
    async function buy() {
        setErr(""); setDone(null);
        if (!selected) { setErr("Pick a token bundle."); return; }
        ...
        <>
            <section className="card" style={{ position: "relative", overflow: "hidden", background: "linear-gradient(160deg, rgba(165,107,232,0.22), rgba(224,85,154,0.12))", border: "1px solid rgba(201,162,255,0.4)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                        <h1 style={{ margin: 0, fontSize: "1.5rem", color: PURPLE }}>Creation Tokens</h1>
                        <p className="muted" style={{ margin: "2px 0 0" }}>Spend a token to <strong style={{ color: PINK }}>design your own AI farm decoration</strong> — describe it, our art pipeline draws it, and it&apos;s <em>yours forever</em> (non-tradeable).</p>
                    </div>
                </div>
                <div style={{ marginTop: 14, display: "inline-flex", alignItems: "center", gap: 10, padding: "8px 14px", borderRadius: 12, background: "rgba(0,0,0,0.25)", border: "1px solid rgba(201,162,255,0.35)" }}>
                    <span className="muted" style={{ fontSize: 13 }}>Your tokens</span>
                ...
                <div style={{ marginTop: 14, display: "flex", flexWrap: "wrap", gap: 8, fontSize: 12.5 }}>
                    <span style={{ padding: "4px 10px", borderRadius: 999, background: "rgba(201,162,255,0.16)", color: "#e7d4ff", fontWeight: 700 }}>✨ Tokens + 🪙 coins in one buy</span>
                    <span style={{ padding: "4px 10px", borderRadius: 999, background: "rgba(255,158,194,0.16)", color: "#ffd6e7", fontWeight: 700 }}>♾️ Your decoration, kept forever</span>
            ...
            {done ? (
                <section className="card" style={{ textAlign: "center", border: "1px solid rgba(201,162,255,0.45)" }}>
                    ...
                    <h2 style={{ margin: "4px 0", color: PURPLE }}>{done.owner ? "Granted" : "Purchased"} {done.tokens} creation token{done.tokens === 1 ? "" : "s"}!</h2>
                    <p className="muted" style={{ marginTop: 0 }}>+{done.coins.toLocaleString()} 🪙 coins dropped into your wallet. You now have <strong style={{ color: PURPLE }}>✨ {tokenBalance.toLocaleString()}</strong> tokens.</p>
                ...
                <section className="card">
                    ...
                    <p className="muted" style={{ marginTop: 0, fontSize: 13 }}>Bigger bundles are a better deal — more tokens and more coins per dollar.</p>
                        ...
                        {tiers.map((t) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div style={{ fontSize: "1.3rem", fontWeight: 900, color: active ? PURPLE : "inherit" }}>{usd(t.priceCents)}</div>
                                    <div style={{ marginTop: 6, fontWeight: 800, color: "#efe3ff" }}>✨ {t.tokens} tokens</div>
                    ...
                    {selected ? (
                        <div style={{ marginTop: 16, padding: 14, borderRadius: 12, background: "rgba(201,162,255,0.08)", border: "1px solid rgba(201,162,255,0.25)" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}><span className="muted">Creation tokens</span><strong style={{ color: PURPLE }}>✨ {selected.tokens}</strong></div>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(201,162,255,0.2)" }}><span style={{ fontWeight: 800 }}>You pay</span><strong>{usd(selected.priceCents)}</strong></div>
                        </div>
                    ) : null}

                    {!paymentsEnabled ? (
                        <p className="muted" style={{ marginTop: 14 }}>Online purchases are currently unavailable. Check back soon to stock up on creation tokens.</p>
                    ) : missingSquareConfig ? (
            ...
            <section className="card">
                <h2 style={{ marginTop: 0 }}>How creation tokens work</h2>
                <ol className="muted" style={{ margin: 0, paddingLeft: 18, fontSize: 13.5, lineHeight: 1.7 }}>
                    <li>Open <strong style={{ color: PURPLE }}>Make your own</strong> in your farm decorations.</li>
                    <li>Describe a decoration — the art pipeline draws it. Not quite right? Add a tweak and it redraws.</li>
                    <li>Pick your favorite. It&apos;s placed in your farm, <strong>yours forever</strong>, and never tradeable.</li>
                    <li>Each design spends <strong>1 token</strong>. Coins you earn on the buy are yours to spend anywhere in the game.</li>
                </ol>
            </section>
        </>
    );

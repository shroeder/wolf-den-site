// Battle sprites for the four CARD-GAME pets — the base sprite plus all four evolved forms (Lv2-5),
// because a pet that levels has to visibly evolve or the levelling means nothing.
//
// ⚠️ THE PROMPTS ARE NOT WRITTEN HERE. They come out of collectibles.js, where the pet is already defined
// with a spritePrompt beside its name, rarity and hint. The fishing script that this is modelled on keeps
// its own copy of four prompts, and a second copy of a description is a second thing to forget when the
// animal is redrawn. One source, read at run time.
//
...
//
//   node scripts/gen-cards-pet-sprites.mjs                 all four, base + Lv2-5 (20 images)
//   node scripts/gen-cards-pet-sprites.mjs chalk_hare      just one
...
// Kept in step with the `source: "fishing"` entries in collectibles.js.
// ⚠️ READ OUT OF THE SOURCE AS TEXT, NOT IMPORTED. collectibles.js and cards-kit.js both use the "@/"
// alias, which plain node cannot resolve — which is exactly why the fishing script this is modelled on
// keeps its own copy of four prompts. Parsing the file is the pattern gen-pet-level6.mjs already uses, and
// it keeps ONE source of truth: adding a fifth pet to LEVEL_PETS needs no edit here at all.
const readSrc = (f) => fs.readFileSync(new URL(f, import.meta.url), "utf8");
const KIT = readSrc("../src/lib/marketplace/cards-kit.js");
const COLL = readSrc("../src/lib/marketplace/collectibles.js");

const levelPetsLine = KIT.match(/export const LEVEL_PETS = \{([^}]*)\}/)?.[1] || "";
const CARD_PET_IDS = [...levelPetsLine.matchAll(/"([a-z_0-9]+)"/g)].map((m) => m[1]);
if (!CARD_PET_IDS.length) throw new Error("LEVEL_PETS not found in cards-kit.js");

const PETS = Object.fromEntries(CARD_PET_IDS.map((id) => {
    // [^] rather than [\s\S]: this regex is built from a string, and a backslash class does not survive
    // being written into one. The first version came out as [sS] and matched nothing at all, silently.
    const re = new RegExp('{ id: "' + id + '",[^]{0,900}?spritePrompt: "([^"]+)"');
    const prompt = COLL.match(re)?.[1];
    if (!prompt) throw new Error(`no spritePrompt for ${id} in collectibles.js`);
    return [id, prompt];
}));
